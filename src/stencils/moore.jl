...
struct Moore{R,N,L,T} <: Stencil{R,N,L,T}
    ...
    Moore{R,N,L,T}(neighbors::SVector{L,T}) where {R,N,L,T} = new{R,N,L,T}(neighbors)
end
Moore{R,N,L}(neighbors::SVector{L,T}) where {R,N,L,T} = Moore{R,N,L,T}(neighbors)
Moore{R,N,L}() where {R,N,L} = Moore{R,N,L}(SVector(ntuple(_ -> nothing, L)))
function Moore{R,N}(args::SVector...) where {R,N}
    L = (2R + 1)^N - 1
    Moore{R,N,L}(args...)
end
Moore{R}(args::SVector...) where R = Moore{R,2}(args...)
Moore(args::SVector...; radius=1, ndims=2) = Moore{radius,ndims}(args...)
